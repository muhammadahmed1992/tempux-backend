...
export class ResetPasswordEmailCreator extends EmailCreator {
  ...
  public factoryMethod(data: {
    ...
    return {
      to: data.to,
      subject: "Your One-Time Password (OTP) - Password Reset Request",
      body:
        `
...
<h1 style="color: #007bff; font-size: 24px; margin: 20px 0; padding: 10px 20px; background-color: #f0f0f0; border-radius: 5px; display: inline-block;">${
          ...
          <p>You have requested a password reset. Please click the link below to reset your password by entering above OTP:</p>
          <p><a href="` +
        baseUrl +
        `reset-password/` +
        data.resetToken +
        `" style="color: #007bff;">Reset Your Password</a></p>
